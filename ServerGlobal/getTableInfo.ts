...
function getElementInfo(gr: GlideRecord): string {
    let lines: string[] = [];
    let tableName: string = gr.getTableName();
    var tGr: GlideRecord = new GlideRecord('sys_db_object');
    tGr.addQuery('name', tableName);
    tGr.query();
    tGr.next();
    let r: string = JSON.stringify(tableName) + ":{\n\t" +
    ...
    tGr.query();
    while (tGr.next()) {
        let name: string = "" + tGr.element;
        if (name.length == 0)
            continue;
        let element: IGlideElement = gr[name];
        let jsType: string = typeof element;
        let e: IElementInfo;
        let d: GlideElementDescriptor;
        let cGr: GlideRecord;
        let ca: IChoiceInfo[] = [];
        if (jsType === "object") {
            if (element === null)
                ...
                d = element.getED();
                let jsClassName: string = Object.prototype.toString.call(element);
                ...
                let isJavaObject: boolean = JSUtil.isJavaObject(element);
                if (isJavaObject) {
                    e.isJavaObject = true;
                ...
                while (cGr.next()) {
                    ...
                    let cs: string = "" + cGr.dependent_value;
                    if (cs.length > 0)
